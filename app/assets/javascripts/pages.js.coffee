...
jQuery ->
  ...
  $("#page_path").find('p').on 'click', ->
    ...
    $('#hide_link').on 'click', (e)->
      parent_path = $("#parent_id").find(':selected').data('path')
      new_url_path = 
        if parent_path is undefined
          '/' + $("#permalink").val()
        else
          '/' + parent_path + '/' + $("#permalink").val()

      $(this).parent().hide()
      $("#page_path").find('p').text(new_url_path).show()
  upArrow = '<a id="cke_1_toolbar_collapser" tabindex="-1" class="cke_toolbox_collapser"><span class="cke_arrow">▲</span></a>'
  ...
  $('#editable').on 'mouseenter', ->
    $('#cke_1_toolbox').children('span:lt(8)').hide()
    $(this).off 'mouseenter'
  $('#editable').on 'focus', ->
    $('#save-editable').show().button('reset').removeClass('btn-success')
    ...
    $('#cke_1_toolbar_collapser').on "click", ->
      $('#cke_1_toolbox').children('span:lt(8)').toggle()
